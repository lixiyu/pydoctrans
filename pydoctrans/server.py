...
GRACE_PERIOD = int(os.environ.get("GRACE_PERIOD", "30"))  # 优雅关闭最长等待秒数
...
def _track_request(format: str, file_size: int):
    ...
    try:
        ...
        request_duration_seconds.labels(to=format).observe(elapsed)
...
async def convert(
    ...
    if not file.filename:
        requests_total.labels(status="400", to=to_).inc()
        raise HTTPException(
            status_code=400,
            detail="缺少文件名",
        )
    ...
    with _track_request(format=to_, file_size=file_len):
        try:
            ...
            requests_total.labels(status="504", to=to_).inc()
            raise HTTPException(status_code=504, detail=str(e)) from e

    requests_total.labels(status="200", to=to_).inc()
